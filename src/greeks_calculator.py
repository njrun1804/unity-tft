...
def integrate_with_pipeline(options_df, stock_quotes_df):
    """
    Integration function for your automate_pipeline.py
    
    Usage in your pipeline:
    from src.greeks_calculator import integrate_with_pipeline
    options_df = integrate_with_pipeline(options_df, stock_quotes_df)
    """
    calculator = GreeksCalculator(risk_free_rate=0.05)
    
    # Merge stock prices with options
    if 'ticker' in options_df.columns and 'ticker' in stock_quotes_df.columns:
        options_df = options_df.merge(
            stock_quotes_df[['ticker', 'price']].drop_duplicates('ticker'),
            on='ticker',
            how='left',
            suffixes=('', '_stock')
        )
    ...
    options_df = calculator.process_options_dataframe(options_df)
    
    # Prepare LSTM features
    options_df = calculator.prepare_lstm_features(options_df)
